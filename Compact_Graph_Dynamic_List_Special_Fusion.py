    ...
    for i in range(5):
        ...
        for gnodes in subgraphs:
            ...
            while len(alloca_incomplete_nodes):
                ...
                neigh_graph_nodes_unalloca_begin = []
                neigh_graph_nodes_unalloca_end = []
                    ...
                    if gnode not in list(alloca_nodes.keys()):
                        if gnode in cur_layer_nodes:
                            if graph[alloca_node][gnode]['con_qubits'][alloca_node] == 0:
                                neigh_graph_nodes_unalloca_begin.append(gnode)
                                begin_qubit.add(graph[alloca_node][gnode]['con_qubits'][alloca_node])
                            elif graph[alloca_node][gnode]['con_qubits'][alloca_node] == MaxDegree - 1:
                                neigh_graph_nodes_unalloca_end.append(gnode)
                            else:
                ...
                neigh_graph_nodes_unalloca = neigh_graph_nodes_unalloca_begin.copy()
                ...
                if search_node.f:
                    if len(search_node.path) == 1:
                        neigh_graph_nodes_unalloca = neigh_graph_nodes_unalloca + neigh_graph_nodes_unalloca_with_one + neigh_graph_nodes_unalloca_end
                    net = search_node.net
                ...
                # allocate the unallocated neighbor nodes
                if len(search_node.path) == 1:
                    pre_neigh_unalloca_nodes = neigh_graph_nodes_unalloca.copy()
                else:
                    pre_neigh_unalloca_nodes = neigh_graph_nodes_unalloca.copy() + neigh_graph_nodes_unalloca_with_one + neigh_graph_nodes_unalloca_end
                neigh_graph_nodes_unalloca = []
                for nnode in neigh_graph_nodes_unalloca_end:
                    if nnode not in alloca_nodes.keys():
                        neigh_graph_nodes_unalloca.append(nnode)
                neigh_graph_nodes_unalloca_size = len(neigh_graph_nodes_unalloca)
                ...
                if search_node.f:
                    if len(search_node.path) == 1:
                        neigh_graph_nodes_unalloca = pre_neigh_unalloca_nodes
                    net = search_node.net
                    count_pos_untake_list = list(search_node.free_space)
                    # allocate the nodes
                    pre_nodes = []
                    ...
                    while len(count_pos_untake_list) and len(neigh_graph_nodes_unalloca):
                        ...
                        if max_used_times_tri == 0:
                            ...
                            if pre_pos != alloca_nodes[alloca_node]:
                                ...
                                print(graph[alloca_node][unalloca_node]['con_qubits'][alloca_node])
                                net[pre_pos][untake_pos]['con_qubits'].append({pre_pnode: graph[alloca_node][unalloca_node]['con_qubits'][alloca_node], untake_pos: graph[alloca_node][unalloca_node]['con_qubits'][unalloca_node]})
                        else:
                            net[pre_pos][untake_pos]['con_qubits'] = []
                            if pre_pos != alloca_nodes[alloca_node]:
...
def compact_graph_dynamic_list_special_fusion(fgraph, dgraph, MaxDegree):
