...
class Pattern:
    ...
    def print_pattern(self, lim=100, filter=None):
...
def measure_pauli(pattern, leave_input, copy=False, use_rustworkx=False):
    ...
    # Obtaining standardised graph of pattern
    nodes, edges = pattern.get_graph()

    # Initialising graph state in graph/ simulation tool
    ...
    # Identifying nodes that can be simulated
    to_measure, non_pauli_meas = pauli_nodes(pattern, leave_input)

    # If we don't want to leave inputs alone, i.e. is False, i.e. we can change input (can become in/out)
    if not leave_input and len(list(set(pattern.input_nodes) & set([i[0][1] for i in to_measure]))) > 0:
        new_inputs = None
    # If we do want to leave inputs alone, i.e. is True.
    else:
        new_inputs = pattern.input_nodes

    # Looking at commands associated with nodes that can be simulated
    for cmd in to_measure:
        # Example of a cmd: (   ['M', 8, 'XY', 0, [3], [2]]    ,      '+X')
        # The measurement basis is determined by cmd[1], from the circuit. 
        # which can be "+X", "-X", "+Y", "-Y", "+Z", or "-Z".
        # extract signals for adaptive angle.
        if cmd[1] in ["+X", "-X"]:
            s_signal = 0  # X meaurement is not affected by s_signal
            t_signal = np.sum([results[j] for j in cmd[0][5]])   # ['M',     8,'XY', 0, [3],      [2]]

        elif cmd[1] in ["+Y", "-Y"]:
            s_signal = np.sum([results[j] for j in cmd[0][4]])   # ['M',     8,'XY', 0,           [3],      [2]]
            t_signal = np.sum([results[j] for j in cmd[0][5]])   # ['M',     8,'XY', 0, [3],      [2]]
        ...
        # Check if s signal is odd, modulus 2
        # unintended flip in the computational basis (from |0⟩ to |1⟩ or vice versa).
        if int(s_signal % 2) == 1:  # equivalent to X byproduct, that needs correcting
            # Adding correction to graph
            graph_state.h(cmd[0][1])
            graph_state.z(cmd[0][1])
            graph_state.h(cmd[0][1])

        # Check if t signal is even, modulus 2
        # unintended phase flip in the quantum state.
        if int(t_signal % 2) == 1:  # equivalent to Z byproduct, that needs correcting
            # Adding correction to graph
            graph_state.z(cmd[0][1])

        # Performing the measurement of cmd
        if cmd[1] == "+X":
            results[cmd[0][1]] = graph_state.measure_x(cmd[0][1], choice=0)
        elif cmd[1] == "-X":
            results[cmd[0][1]] = 1 - graph_state.measure_x(cmd[0][1], choice=1)  # Inverting result with 1 - 
            
        elif cmd[1] == "+Y":
            results[cmd[0][1]] = graph_state.measure_y(cmd[0][1], choice=0)
        elif cmd[1] == "-Y":
            results[cmd[0][1]] = 1 - graph_state.measure_y(cmd[0][1], choice=1) # Inverting result with 1 - 

        elif cmd[1] == "+Z":
            results[cmd[0][1]] = graph_state.measure_z(cmd[0][1], choice=0)
        elif cmd[1] == "-Z":
            results[cmd[0][1]] = 1 - graph_state.measure_z(cmd[0][1], choice=1) # Inverting result with 1 - 
    ...
    # which means we can just choose s=0. 
    isolates = graph_state.get_isolates()
    for node in non_pauli_meas:
        if (node in isolates) and (node not in pattern.output_nodes): # We should not remove output nodes even if isolated.
            graph_state.remove_node(node)
            results[node] = 0

    # Update command sequence
            
    # Getting new vertex operations dictionary.
    # Here commands on a node are mapped to required local clifford operators
    vops = graph_state.get_vops() 


    new_seq = [] # Initialising new sequence

    # Reconstructing the graph state
    for index in iter(graph_state.nodes):
        new_seq.append(["N", index])
    for edge in iter(graph_state.edges):
        new_seq.append(["E", edge])
    
    # Comparing vops with original command list
    for cmd in pattern.seq:
        # Altering measurements 
        if cmd[0] == "M": # Command is a measurement
            # If command exists in new list of nodes...
            if cmd[1] in list(graph_state.nodes):
                cmd_new = deepcopy(cmd) # Initialising new command

                new_clifford_ = vops[cmd[1]]  # Obtaining dictionary 

                if len(cmd_new) == 7: # If command length has already been extended then equate to dictionary
                    cmd_new[6] = new_clifford_  
                else: # If command length was not extended then add to dictionary. 
